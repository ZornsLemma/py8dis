...
def pseudopc_end(dest, source, length):
    move_id = movemanager.move_id_for_binary_addr[source]
    return [utils.LazyString("* = %s + (%s - %s)", disassembly.get_label(source, source, move_id), disassembly.get_label(dest + length, source, move_id), disassembly.get_label(dest, source, move_id))]
