...
def suggest_label_name(runtime_addr, binary_addr, move_id):
    ...
    lmd = labelmakingdata.LabelMakingData("",                       # name (to be filled in below)
                                          ...
                                          binary_addr,              # binary_addr is where the label is used
...
def label_maker(runtime_addr, binary_addr, move_id):
    ...
    # binary_addr is an address where the label is being used (e.g. the address of an
    # operand of an instruction like 'LDA label').
    lmd = suggest_label_name(runtime_addr, binary_addr, move_id)

    # Default helper possibly updating the suggestion
...
def get_final_label(runtime_addr, binary_addr, move_id):
    ...
    assert memorymanager.is_valid_binary_addr(binary_addr)
    assert move_id is None or movemanager.is_valid_move_id(move_id)
    lmd = label_maker(runtime_addr, binary_addr, move_id)
