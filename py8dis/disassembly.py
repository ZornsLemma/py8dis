...
def get_label(runtime_addr, context, move_id=None):
    ...
    context = memorymanager.BinaryAddr(context) # TODO: I think this is how things currently work, arguably since the context is passed to user code we should be using RuntimeAddr for context (at least by the time it gets out to user code, maybe not right here)
...
def our_label_maker(runtime_addr, context, move_id):
    ...
    runtime_addr = memorymanager.RuntimeAddr(runtime_addr) # TODO: OK? "type" for label addresses is a bit unclear
