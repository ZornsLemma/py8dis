...
def our_label_maker(runtime_addr, context, move_id):
    """Return a label name and move_id, auto-creating one if needed.
    ...
    if move_id is None:
        ...
        # TODO: Next bit TCO (and move_ids2 is unused), not sure if
        # this is good or not
        #if move_id not in move_ids2:
        #    move_id = movemanager.base_move_id

        # TODO: Alternative experimental thing - the basic idea is that
        # if we can't assign a move ID based on a matching of context
        # and runtime_addr move IDs, *if* there is any existing label
        # for any of those move IDs we will use it rather than forcing
        # a new label in base_move_id to be created
        #
        # TODO: but should we actually require *THERE IS ONLY ONE SUCH
        # LABEL* - if there are multiple ones, we have no basis for
        # picking one over another (I think)
        if move_id not in move_ids2:
    ...
    if len(label.all_names()) > 0:
        # We are just returning the first name arbitrarily, which is
        # probably fine, we have no basis to choose anything else.

        # We look for a label/expression in the current move_id, or
        # failing that in the base_move_id.

        # TODO: using loops here seems odd
...
def split_classification(binary_addr):
    ...
    classifications[split_addr] = classification.Byte(classifications[binary_addr].length() - first_split_length, False)
    classifications[binary_addr] = classification.Byte(first_split_length, False)
